//=====================
//HTML code for webpage
//=====================
const char main_webpage[] PROGMEM = 
R"=====(
<!DOCTYPE html>
<html lang='en'>
  <head>
    <meta charset=utf-8>
      <title>ESP8266 Powermeter</title>
    </head>
    <body>
<style>
    html, body {
        font-family: Trebuchet MS, sans-serif;
        font-size: 16px;
        text-align: center;
        color: #3399FF;
        width: 100%;
        height:100%;
        background: linear-gradient(0deg,#1e2730,#1e2730,#1e2730, #717171);
        background-size: 400% 400%;
        display: table;
        position: relative;
    }
    .lines {
      position: absolute;
      top: 0;
      left: 0;
      right: 0;
      height: 10%;
      margin: auto;
      width: 100%;
    }
    .line {
      position: absolute;
      width: 110%;
      height: 1px;
      top: 0;
      left: -5%;
      background: rgba(255, 255, 255, 0.1);
      overflow: hidden;
    }
    .line::after {
      content: '';
      display: block;
      position: absolute;
      height: 100%;
      width: 15vh;
      top: 0%;
      left: 0;
      background: linear-gradient(to right, rgba(255, 255, 255, 0) 0%, #ffffff 75%, #ffffff 100%);
        animation: drop 7s 0s infinite;
        animation-fill-mode: forwards;
        animation-timing-function: cubic-bezier(0.4, 0.26, 0, 0.97);
    }
    @keyframes drop {0% {left: -50%;} 100% {left: 110%;}}
    canvas{border-radius: 15px;
    }
    .box{
          display: flex;
          justify-content: center;
          max-width: 100%;
          padding: 130px 0px 0px 0px;
    }
    .box-text{
          width: 50%;
          float: left;
          padding: 25px 35px 5px 35px;
          max-height: 370px;
          min-height: 370px;
          display: inline-block;
          margin: 10px 10px;
          background-color: #fff;
          box-shadow: 0 27px 87px rgba(0,0,0,0.5);
          border-radius: 15px;
          overflow: hidden;
    }
    .box-text p{
        font-size: 14px;
        font-weight: 400;
        text-align: left;
        color: #696969;
        line-height: 3.5px;
        letter-spacing: 0px;
    }
    .box-icons{
          width: calc(50% + 305px);
          justify-content: right;
          border: 0px transparent;
          display: inline-block;
    }
    .logo {
        position: absolute;
        top: 0px;
        left: calc(50% - 90px);
        padding: 5px 0px 0px 0px;
    }
    .title{
        position: absolute;
        width: 326px;
        top: 0px;
        left: calc(50% - 163px);
        padding: 70px 0px 0px 0px;
    }
    .setup_bt {
      width:90px;
      height:32px;
      float: right;
      color: #696969;
      cursor: pointer;
      text-align: right;
      border: 0px transparent;
      background:
      url('data:image/png;base64,')
      no-repeat
      left center;
      padding: 0px 10px 0px 0px;
      background-size: 32px;
    }
    .firmware_bt {
      width:100px;
      height:32px;
      float: right;
      color: #696969;
      cursor: pointer;
      text-align: right;
      border: 0px transparent;
      background:
      url('data:image/png;base64,')
      no-repeat
      left center;
      padding: 0px 0px 0px 0px;
      background-size: 32px;
    }
    .firmware_bt:hover {
      color: #337ab7;
    }
    .setup_bt:hover {
      color: #337ab7;
    }
</style>
 <img class="logo" src='data:image/png;base64,'>
<h1 class='title'>My Home PowerMeter</h1>
 <div class='box'>
  <div class='box-text' id='b-t'>
    <div class='lines'>
      <div class='line'></div>
    </div>

    <canvas class='canvas' id='canvas' width='200' height='200' style='border:1px solid #d3d3d3;'> Your browser does not support the HTML5 canvas tag
        </canvas>
  </div>
 </div>
 <div class='box-icons'>
    <form  class='firmware-icons' action='/firmware'>
        <button class='firmware_bt' href='/firmware' type='submit'>
            <img class='firmware_icon'/>Firmware<br>Update
        </button>
    </form>
    <form  class='setup-icons' action='/setup'>
        <button class='setup_bt' href='/setup' type='submit'>
            <img class='firmware_icon'/>System<br>Setup
        </button>
    </form>
</div>
<script>
    var elem = document.getElementById('b-t');
    var properties = window.getComputedStyle(elem, null);
    var a_width = properties.width;

    var connection = new WebSocket('ws://'+location.hostname+':81/', ['arduino']);
    connection.onopen = function () {
        connection.send('Conectado  -  ' + new Date());
        console.log('Conectado  -  ' + new Date());
    }
        connection.onmessage = function (event) {
        console.log('Servidor (recibe): ', event.data);
        verValor();
    }
        connection.onerror = function (error) {
        console.log('WebSocket Error!!!', error);
    }
    function verValor() {
    var data = event.data;
    var tmp = data.split('#');
    var offset=30;
    var offset_ph=3.75;

    var a = document.getElementById('canvas');
    var atxh = a.getContext('2d');
    var t_height = 350;
    var t_width = a_width.substring(0,3);
    a.setAttribute('width',t_width);
    a.setAttribute('height',t_height);
    var P_power = 0.2*t_width;
    var S_power = tmp[4]*(P_power)/tmp[3];
    var Q_power = tmp[5]*(P_power)/tmp[3];
    var V_phasor=(t_height/2-20)*tmp[0]/250;
    var I_phasor=(t_height/2-20)*tmp[1]/1.5;
    var angle = Math.round((180/Math.PI)*Math.acos(tmp[3]/tmp[4])*100)/100;
    var t_gradient = atxh.createLinearGradient(0, 0, 170, 0);
    t_gradient.addColorStop('0', '#0ff');
    t_gradient.addColorStop('0.5' ,'#33CCFF');
    t_gradient.addColorStop('1.0', '#3399FF');
        
    atxh.clearRect(0, 0, t_width, t_height)
    atxh.strokeRect(0, 0, t_width, t_height)
    atxh.fill();
    canvas.style.backgroundColor = '#1d1f20';
    atxh.strokeStyle = '#33CCFF';
    atxh.lineWidth=2;
    atxh.beginPath();
    atxh.moveTo(P_power+offset,40);
    atxh.lineTo(25+offset, 40);
    atxh.moveTo(P_power+offset,40+Q_power);
    atxh.lineTo(P_power+offset,40);
    atxh.moveTo(25+offset,40);
    atxh.lineTo(P_power+offset,40+Q_power);
    atxh.stroke();
    atxh.beginPath();
    atxh.arc(25+offset,40,30,0,angle*Math.PI/180);
    atxh.stroke();

    atxh.beginPath();
    atxh.arc(offset_ph*P_power,t_height/2,(t_height/2)-20,Math.PI,-1*Math.PI);
    atxh.stroke();
    atxh.strokeStyle = '#4B4B4B';
    atxh.beginPath();
    for(var g = 0; g < 37; g++){
        atxh.moveTo(offset_ph*P_power,t_height/2);
        atxh.lineTo(offset_ph*P_power+((t_height/2)-20+10)*Math.cos(10*g*Math.PI/180),t_height/2+(t_height/2-20+10)*Math.sin(10*g*Math.PI/180));
    }
    atxh.stroke();
    atxh.strokeStyle = '#909090';
    atxh.beginPath();
    for(var r = 0; r < 4; r++){
        atxh.moveTo(offset_ph*P_power,t_height/2);
        atxh.lineTo(offset_ph*P_power+((t_height/2)-20+10)*Math.cos(90*r*Math.PI/180),t_height/2+(t_height/2-20+10)*Math.sin(90*r*Math.PI/180));
    }
    atxh.stroke();
    atxh.strokeStyle = '#0108FF';
    atxh.beginPath();
    atxh.moveTo(offset_ph*P_power,t_height/2);
    atxh.lineTo(offset_ph*P_power+V_phasor,t_height/2+0);
    atxh.stroke();
    atxh.beginPath();
    atxh.lineWidth=6;
    atxh.arc(offset_ph*P_power+V_phasor,t_height/2,3,Math.PI,-1*Math.PI);
    atxh.stroke();
    atxh.strokeStyle = '#8600FC';
    atxh.lineWidth=2;
    atxh.beginPath();
    atxh.moveTo(offset_ph*P_power,t_height/2);
    atxh.lineTo(offset_ph*P_power+I_phasor*Math.cos(angle*Math.PI/180),t_height/2+I_phasor*Math.sin(angle*Math.PI/180));
    atxh.stroke();
    atxh.beginPath();
    atxh.lineWidth=6;
    atxh.arc(offset_ph*P_power+I_phasor*Math.cos(angle*Math.PI/180),t_height/2+I_phasor*Math.sin(angle*Math.PI/180),3,Math.PI,-1*Math.PI);
    atxh.stroke();
    
    atxh.fillStyle = '#E100FC';
    atxh.shadowColor ='rgba(0,0,0,0)';
    atxh.font = 'bold 20px Arial';
    atxh.textAlign = 'center';
    atxh.fillText(tmp[3]+' W',(P_power+25+offset)/2, 30);
    atxh.textAlign = 'left';
    atxh.fillText(tmp[5]+' var',P_power+10+offset, 40+Q_power/2);
    atxh.textAlign = 'right';
    atxh.fillText(tmp[4]+' va',(P_power+45+offset)/2-20, 40+Q_power/2);
    atxh.textAlign = 'left';
    atxh.fillText(angle+'°',58+offset, 58);
    atxh.fillText(tmp[0]+'V',offset_ph*P_power+V_phasor+10,t_height/2);
    atxh.fillText(tmp[1]+'A',offset_ph*P_power+I_phasor*Math.cos(angle*Math.PI/180)+10,t_height/2+I_phasor*Math.sin(angle*Math.PI/180)+10);
    atxh.stroke();
    }
</script>
</body>
</hmtl>
)=====";
